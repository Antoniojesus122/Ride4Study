...
<body class="min-h-full text-text relative overflow-x-hidden">
    ...
    <div class="absolute -top-40 -left-20 w-[500px] h-[500px] bg-red-500/10 rounded-full blur-3xl pointer-events-none blob"></div>
    <div class="absolute -bottom-40 -right-20 w-[500px] h-[500px] bg-amber-500/10 rounded-full blur-3xl pointer-events-none blob" style="animation-delay: -7s;"></div>
    ...
    <div class="relative min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 py-10">
        ...
        <!-- Badge de tipo "Admin" -->
        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r from-red-500/15 to-amber-500/10 border border-red-400/30 rounded-full text-red-300 text-[11px] font-bold uppercase tracking-widest mb-6 shadow-lg shadow-red-500/10">
            <i class="fas fa-user-shield text-[10px]" aria-hidden="true"></i> Panel de administración
        </div>
        ...
        <div class="w-full max-w-md">
            <div class="bg-gradient-to-b from-surface to-surface/80 rounded-2xl shadow-2xl ring-1 ring-white/10 backdrop-blur-sm p-7 sm:p-9">
                ...
                <div class="flex justify-center mb-5">
                    <div class="relative">
                        <div class="absolute inset-0 bg-red-500/30 rounded-2xl blur-xl animate-pulse"></div>
                        <div class="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500/20 to-amber-400/10 border border-red-400/30 flex items-center justify-center shadow-lg">
                            <i class="fas fa-shield-halved text-red-300 text-2xl" aria-hidden="true"></i>
                        </div>
                    </div>
                </div>

                <div class="text-center mb-6">
                    <h1 class="text-3xl font-extrabold text-white tracking-tight"><?= t('auth.2fa_title') ?></h1>
                    <p class="mt-3 text-base text-gray-400 leading-relaxed"><?= t('auth.2fa_subtitle') ?></p>
                    <div class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-sm text-gray-300">
                        <i class="fas fa-envelope text-red-400/70 text-[10px]" aria-hidden="true"></i>
                ...
                <form method="POST" action="<?= url('/admin-verify') ?>" id="verifyForm">
                    ...
                    <div class="flex items-center justify-center gap-1.5 mb-6">
                        <?php for ($j = 0; $j < 5; $j++): ?>
                            <div class="w-2 h-2 rounded-full <?= $j < $attemptsLeft ? 'bg-red-400 shadow-sm shadow-red-400/50' : 'bg-gray-700' ?> transition-colors"></div>
                        <?php endfor; ?>
                        <span class="text-xs text-gray-400 ml-2"><?= $attemptsLeft ?> <?= t('auth.2fa_attempts') ?></span>
                    </div>

                    <button type="submit" id="submitBtn" disabled
                            class="flex w-full justify-center items-center gap-2 rounded-xl bg-red-500/30 px-3 py-3 text-base font-bold text-white/50 cursor-not-allowed transition-all">
                ...
                <div class="mt-6 pt-5 border-t border-white/5 text-center">
                    <a href="<?= url('/login') ?>" class="inline-flex items-center gap-1.5 text-base text-gray-400 hover:text-primary transition-colors">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const enabledClasses = 'flex w-full justify-center items-center gap-2 rounded-xl bg-gradient-to-r from-red-500 to-amber-500 px-3 py-3 text-base font-bold text-white shadow-lg shadow-red-500/25 hover:shadow-xl hover:shadow-red-500/40 hover:-translate-y-0.5 active:translate-y-0 transition-all cursor-pointer';
            const disabledClasses = 'flex w-full justify-center items-center gap-2 rounded-xl bg-red-500/30 px-3 py-3 text-base font-bold text-white/50 cursor-not-allowed transition-all';
