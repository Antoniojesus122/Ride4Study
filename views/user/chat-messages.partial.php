                ...
                <div class="flex items-center justify-end gap-1.5 mt-1 opacity-60 text-[10px]">
                    ...
                    <?php if ($isMe && (time() - strtotime($msg['fechaCreacion']) < 3600)): ?>
                        <div class="hidden group-hover:flex gap-2 ml-2 border-l border-black/20 pl-2">
                            <button onclick="editMessage(<?= $msg['idMensaje'] ?>)" class="hover:text-white"><i class="fas fa-pen"></i></button>
                            <button onclick="deleteMessage(<?= $msg['idMensaje'] ?>)" class="hover:text-red-900"><i class="fas fa-trash"></i></button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
