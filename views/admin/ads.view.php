...
<main class="md:ml-[72px] flex-1 min-w-0 min-h-screen flex flex-col">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <p class="text-sm sm:text-base text-gray-400"><?= $totalAds ?> anuncios</p>
    ...
    <!-- Filtros -->
    <?php
        $adsAdvFilters = [$filters['tipo'] ?? '', $filters['estado'] ?? '', $filters['precio'] ?? '', $filters['institucion'] ?? '', $filters['date_from'] ?? '', $filters['date_to'] ?? '', $_GET['periodo'] ?? ''];
        $adsActiveAdv = count(array_filter($adsAdvFilters, fn($v) => $v !== '' && $v !== null));
    ?>
    <form method="GET" action="<?= url('/admin/ads') ?>" class="mb-8">
        <div class="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button type="button" onclick="toggleAdsAdvFilters()"
                    ...
                    aria-expanded="<?= $adsActiveAdv > 0 ? 'true' : 'false' ?>" aria-controls="ads-adv-filters">
                <span class="flex items-center gap-2">
                    <i class="fas fa-sliders text-xs text-primary" aria-hidden="true"></i>
                    Más filtros
                    <?php if ($adsActiveAdv > 0): ?>
                        <span class="bg-primary text-secondary text-[10px] font-bold px-1.5 py-0.5 rounded-full"><?= $adsActiveAdv ?></span>
                    <?php endif; ?>
                </span>
                <i class="fas fa-chevron-down text-xs text-gray-500 transition-transform <?= $adsActiveAdv > 0 ? 'rotate-180' : '' ?>" id="ads-adv-chevron" aria-hidden="true"></i>
            </button>
        </div>

        <div id="ads-adv-filters" class="<?= $adsActiveAdv > 0 ? '' : 'hidden' ?> flex flex-col sm:!flex-row sm:flex-wrap sm:items-center gap-3 mt-4">
            <select name="tipo" class="px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-sm sm:text-base text-gray-200 focus:outline-none focus:border-primary w-full sm:w-auto">
            ...
            <select name="estado" class="px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-sm sm:text-base text-gray-200 focus:outline-none focus:border-primary w-full sm:w-auto">
            ...
            <select name="precio" class="px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-sm sm:text-base text-gray-200 focus:outline-none focus:border-primary w-full sm:w-auto">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                ...
                <?php if (!empty($hasFilters)): ?>
                    <a href="<?= url('/admin/ads') ?>" class="text-sm text-gray-400 hover:text-gray-200 whitespace-nowrap">Limpiar</a>
                <?php endif; ?>
            </div>
        </div>
    </form>
    <script>
        function toggleAdsAdvFilters() {
            const w = document.getElementById('ads-adv-filters');
            const c = document.getElementById('ads-adv-chevron');
            const b = document.querySelector('[aria-controls="ads-adv-filters"]');
    ...
    <?php else: ?>
        ...
        <?php
            ...
            renderPagination((int)$page, (int)$totalPages, url('/admin/ads'), $adsPagParams);
        ?>
    <?php endif; ?>
