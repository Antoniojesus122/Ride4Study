...
<main class="ml-[72px] flex-1 min-h-screen flex flex-col">
    ...
    <div class="flex-1 p-10 max-w-3xl">
    ...
    <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-7 mb-8">
        <div class="flex items-center gap-4 mb-8">
            <div class="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center">
                <span class="text-primary text-2xl font-bold"><?= mb_strtoupper(mb_substr($adminData['nombre'] ?? 'A', 0, 1)) ?></span>
            </div>
            <div>
                <h2 class="text-xl font-semibold text-white"><?= htmlspecialchars($adminData['nombre'] ?? '') ?></h2>
                <p class="text-base text-gray-400">Administrador</p>
            </div>
        </div>

        <h3 class="text-base font-semibold text-white mb-5">Informacion personal</h3>
        <form method="POST" action="<?= url('/admin/profile') ?>" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nombre" value="<?= htmlspecialchars($adminData['nombre'] ?? '') ?>" required
                       class="w-full px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-base text-gray-200 focus:outline-none focus:border-primary">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Correo electronico</label>
            ...
            <div>
                ...
                <input type="text" name="telefono" value="<?= htmlspecialchars($adminData['telefono'] ?? '') ?>"
                       class="w-full px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-base text-gray-200 placeholder-gray-500 focus:outline-none focus:border-primary" placeholder="Opcional">
            </div>
    ...
    <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-7">
        <h3 class="text-base font-semibold text-white mb-5">Cambiar contrasena</h3>
        <form method="POST" action="<?= url('/admin/profile') ?>" class="space-y-5">
            <input type="hidden" name="action" value="change_password">
            <div>
                <label class="block text-sm text-gray-400 mb-1">Contrasena actual</label>
                <input type="password" name="current_password" required
                       class="w-full px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-base text-gray-200 focus:outline-none focus:border-primary">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Nueva contrasena</label>
                <input type="password" name="new_password" required minlength="6"
                       class="w-full px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-base text-gray-200 focus:outline-none focus:border-primary">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Confirmar nueva contrasena</label>
                <input type="password" name="confirm_password" required minlength="6"
                       class="w-full px-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-base text-gray-200 focus:outline-none focus:border-primary">
            </div>
            <div class="flex justify-end">
                <button type="submit" class="px-5 py-2.5 text-base font-medium bg-gray-700 text-gray-200 rounded-lg hover:bg-gray-600 transition border border-gray-700">
                    Cambiar contrasena
                </button>
            </div>
        </form>
